         ...
         <div class="container">
            ...
            <div class="alert alert-success"> {{ session()->get('success') }} </div> @endif
                        ...
                        <div class="title">
                            ...
                            <br>
                            <h3>كود الوظيفة : {{$job->code}}</h3>
                            <p class="data">{{ date('Y-m-d', strtotime($job->created_at)) }}</p>
                            <p>شرط التقدم للوظيفة :</p>
                            <p>{!! $job->condition !!}</p>
                        </div>
                     ...
                     <div class="modal-content">
                         ...
                         <div class="modal-body">
                             @if(isset ($errors) && count($errors) > 0)
                                 ...
                                 @endforeach
                             @endif
                             <form action={{url('/job-applications')}} method="POST" enctype='multipart/form-data'>
                                 @csrf
                                 @method('POST')
